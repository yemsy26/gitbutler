...
fn move_branch_destination_conflict() {
    ...
    let dest_branch_name = {
        ...
        let vb_state = VirtualBranchesHandle::new(ctx.project_data_dir());
        let dest_stack = vb_state.get_stack_in_workspace(dest_id).unwrap();
        let branch_name = dest_stack.branches()[0].name.clone();
        let merge_base = dest_stack.merge_base(ctx).unwrap();
        let dest_commit =
            super::create_raw_commit(&repo, merge_base, "shared.txt", b"dest\n").unwrap();
        let mut dest_stack = dest_stack;
        let mut vb_state = vb_state;
        dest_stack
            .set_stack_head(&mut vb_state, &repo, dest_commit)
            .unwrap();
        branch_name
    };
...
fn move_branch_preexisting_conflict() {
    ...
    }
    {
        let repo = ctx.repo.get().unwrap();
        let vb_state = VirtualBranchesHandle::new(ctx.project_data_dir());
        let source_stack = vb_state.get_stack_in_workspace(source_id).unwrap();
        let merge_base = source_stack.merge_base(ctx).unwrap();
        ...
        let competitor =
            super::create_raw_commit(&repo, merge_base, "shared.txt", b"competitor\n").unwrap();
        super::push_conflicted_commit(ctx, source_id, commit_to_move, competitor).unwrap();
    }
